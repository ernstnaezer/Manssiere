...
namespace Manssiere.Core.Graphics.Tesselation.Tesselator
{
    public class Dictionary
    {
        ...
        public Node Insert(ActiveRegion k)
        {
            return InsertBefore(head, k);
        }

        public Node InsertBefore(Node node, ActiveRegion key)
        {
            Node newNode;
            ...
            {
                node = node.prev;
            } while (node.Key != null && !ActiveRegion.EdgeLeq(tesseator, node.Key, key));
        ...
        public static Node dictSearch(Dictionary dict, ActiveRegion key)
        {
            Node node = dict.head;
        ...
        public class Node
        {
            private ActiveRegion key = new ActiveRegion();
            public Node next;
            public Node prev;

            public ActiveRegion Key
            {
                get { return key; }
                set { key = value; }
            }
